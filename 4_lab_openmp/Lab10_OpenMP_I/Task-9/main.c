...
int main(int argc, char *argv[])
{
    if (argc !=3)
    {
        printf("Please give 2 argument: N (number of elements to sort) and M (number of threads).\n");
        return -1;
    }
    int N = atoi(argv[1]);
    ...
    merge_sort(list_to_sort, N, 0, n_thr); // 1 mallocs
    // merge_sort(list_to_sort, N, n_thr); // 2 mallocs
